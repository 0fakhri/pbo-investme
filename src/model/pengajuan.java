...
public class pengajuan {
    private static String id, jmlhPinjam, wktPelunasan, wktCicilan, jaminan, buktiKepemilikan, alasanPeminjaman, status, pembuat, idSementara;
...
//                System.out.println(""+getJmlhPinjam());
            }
    ...
    public boolean verif(String Status, String id){
        ...
        try {
            ...
            Statement stmt = koneksi.con.createStatement();
            queryResult = stmt.executeUpdate(String.format(
                    "UPDATE `dataPengajuan` SET `statusVerif`='%s' WHERE idPengajuan='%s'", 
                    status, id)) > 0;
